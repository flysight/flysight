...
{    
    ...
    for (NSUInteger i = 1; i < n; i++) {
		...
		BOOL lastVisible = ((xRangeFlags[i-1] == CPPlotRangeComparisonResultNumberInRange) && 
							(yRangeFlags[i-1] == CPPlotRangeComparisonResultNumberInRange));
		BOOL thisVisible = ((xRangeFlags[i] == CPPlotRangeComparisonResultNumberInRange) && 
							(yRangeFlags[i] == CPPlotRangeComparisonResultNumberInRange));
		if ( !visibleOnly && (lastVisible != thisVisible) ) {
...
{
	// Make sure graphs are clipped even when CPPlotArea's layoutSublayers method isn't called
	if (self.renderingRecursively) CGContextClipToRect(theContext, self.bounds);
